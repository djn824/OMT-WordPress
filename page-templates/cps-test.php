...
<div class="full-container">
	<div class="side_bar_menu menu">
		<div>
			<a href="/cps-test">
				<span data-index="cps" class="active">
	...
	<div id="cps" class="container">
		<div class="click_box">
			<div class="clk_box">
				<div class="clk_area" id="clickarea">
					<button id="start" onclick="start_cps_test()">Click here to start the CPS test</button>
				</div>
		...
		<section class="score_modal" role="dialog">
			...
			<div class="modal_cont">
				...
				<p>
					Your click speed is <b id="endcps">0.20</b> clicks per second (CPS).<br>
					<b id="endclicks">1</b> clicks in <b id="endtimer">5</b> seconds. 
				</p>
				<ul id="endstars">
				</ul>
				<!-- 					<ul id="endstars">
...
<script>
	...
	const start_cps_test = () => {
		let clk_box = cps.querySelector(".clk_box");
		let startBtn = cps.querySelector("#start");

		let timerLabel = cps.querySelector("#timer");
		let clickLabel = cps.querySelector("#clicks");
		let scoreLabel = cps.querySelector("#score");

		let endCps = cps.querySelector("#endcps");
		let endClicks = cps.querySelector("#endclicks");
		let endtimer = cps.querySelector("#endtimer");
		
		let modal = cps.querySelector(".score_modal");
		let close_btn = cps.querySelector(".modal-close");
		let try_btn = cps.querySelector(".modal-try");
		...
		const handleClick = (e) => {
			if(test_started) {
				...
				let rect = clk_box.getBoundingClientRect();
		...
		const end_test = () => {
			endClicks.innerText = count;
			endtimer.innerText = time / 1000;
			endCps.innerText = (count / (time/1000)).toFixed(2);
		...
		let test_timer = setInterval(() => {
			...
			clickLabel.innerText = count !== 0 ? (count / seconds).toFixed(2) : '0.00';
