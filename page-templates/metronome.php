...
get_header();
$metronome_theme_uri = get_stylesheet_directory_uri();
?>
<link rel="preload" as="image" href="<?php echo esc_url( $metronome_theme_uri ); ?>/assets/images/metronome-template-orange.svg" type="image/svg+xml">
<style media="screen">
...
/* display: block; */
	...
	/* Fixed on template pivot — outside needle rotator so it does not swing */
	.weight-point {
        ...
        <div>
			...
			<div id="metronome-sizer">
				...
				<div class="metronome-wrapper">
					<div class="metronome-bg-stack">
						<img class="img-fluid skip-lazy"
							 id="metronome-bg"
							 src="<?=get_stylesheet_directory_uri();?>/assets/images/metronome-template.svg" alt="">
						<img class="img-fluid skip-lazy metronome-bg-flash"
							 id="metronome-bg-orange"
							 src="<?=get_stylesheet_directory_uri();?>/assets/images/metronome-template-orange.svg"
							 alt=""
							 aria-hidden="true">
					</div>
...
// 			beatSelect.value = val;
	...
	function scaleNeedle() {
		...
		// ── Weight point: fixed on template pivot (wrapper coords, aligned to needle base)
		if (pointImg && pointEl && needle) {
	...
	function flashBackground(isStrong) {
		...
		const metronomeBgOrange = document.getElementById('metronome-bg-orange');
		if (!metronomeBgOrange) return;

		metronomeBgOrange.classList.add('is-flash');
		clearTimeout(metronomeFlashTimer);
		metronomeFlashTimer = setTimeout(() => {
			metronomeBgOrange.classList.remove('is-flash');
		}, 250); // ← 250ms flash duration
	}
